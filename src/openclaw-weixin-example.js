...
// ---------- 入站轮询（仅本示例使用；核心能力在 @openagent/core） ----------
...
function extractInboundTextOnly(msg) {
  const items = msg?.item_list;
...
function startWeixinInboundPoller(options = {}) {
      ...
      try {
        ...
        for (const msg of msgs) {
          ...
          if (options.onUserTextMessage && textOnly && ctx && uid) {
            enqueue(() => Promise.resolve(options.onUserTextMessage({ msg, text: textOnly, fromUserId: uid, contextToken: ctx })));
